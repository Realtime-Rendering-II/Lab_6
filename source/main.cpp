...
int width = 800, height = 600, current_lod = 2;
float last_x, last_y, delta_time = 0.0f, last_frame_time = 0.0f;
bool wireframe = false, first_mouse = false, fixed_lod = true, retriangulate = true;
Camera *camera = new Camera{glm::vec3{0.4, 0.5, 2.6}, glm::vec3{0, 1, 0}};
GLFWwindow *window;
...
int main(int argc, const char *argv[]) {
    ...
    Texture* height_map = new Texture{"../textures/high_res.png"};

    unsigned int map_width = 20;
    unsigned int map_length = 20;

    Patches* patches = new Patches{height_map->get_texture(), map_width, map_length};
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        patches->update_lod(camera->Position, fixed_lod, current_lod);

        patches->draw(normal_shader.get_program(), retriangulate);
        ...
        if(fixed_lod) {
            ImGui::Text("Key[3] Global LOD: activated");
            ImGui::Text("Key[2] Current LOD: %d", current_lod);
        }
        else
            ImGui::Text("Key[3] Global LOD: deactivated");
        if(retriangulate)
            ImGui::Text("Key[4] Retriangulation: activated");
        else
            ImGui::Text("Key[4] Retriangulation: deactivated");
...
void initialize() {
    ...
    */
    // create the window
    window = glfwCreateWindow(width, height, "Lab 6", NULL, NULL);
